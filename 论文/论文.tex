...
(2)提出了一种基于$Jacobi-proximal ADMM$框架的分布式方法来求解该模型。背后的主要思想是解耦耦合约束将原始模型分解为一组小尺度的问题，并行求解该问题，不仅保护了EV用户的隐私性，用从理论上证明了该算法的最优性和收敛性

(3)相比ADMM，其优势是：具有更快的收敛性，Jacobi-proximal ADMM在每个迭代步骤中同时更新变量，而不是按顺序逐个更新。这使得Jacobi-proximal ADMM能够更快地收敛到最优解。Jacobi-proximal ADMM通过同时更新变量，可以减少数值误差的传播。这在面对数值不稳定问题时尤为重要，能够提供更可靠的数值结果。

(3)由于使用$Jacobi-proximal ADMM$框架的分布式方法来求解该模型，并行求解该模型
\subsection{文献综述}
...
%\begin{figure}[!ht]\underbrace{}
%%    \centering
%    \includegraphics[width=5cm]{price.png}
