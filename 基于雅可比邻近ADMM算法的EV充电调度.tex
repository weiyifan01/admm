...
2）设计了一种新的基于~$ADMM-PJacobi$~的算法来解决具有时间空间耦合不等式约束的不可分优化问题，并从理论上证明了该算法的最优性和收敛性，从理论和数值实验上证明了与经典的~$ADMM$~相比收敛速度快
...
\begin{align*}
\frac{C_n}{\eta_n^+ \Delta t}&(SOC_n^{min}-SOC_n^{init})\leq \sum_{t=1}^T p_{n,t}\\\tag{3}
& \leq \frac{C_n}{\eta_n^+\Delta t}(SOC_n^{max}-S_n^{init})
\end{align*}
其中~$SOC_{init}$~是~$EV~n$~的初始~$SOC$~值.~
...
\end{align*}
在实践中,~一般采用以下保守约束,即每个~$EV n$~在有限时间范围~$T$~结束时至少达到最终SOC值,~表示为~$SOC_n^{final}$,
\begin{align}\tag{6}
\sum_{t=1}^Tp_{n,t}\geq \frac{C_n}{q_n^+\Delta t}(SOC_n^{final}-SOC_n^{init}).
\end{align}
为了简化表示,~定义~$\mathcal{P}_n^{(1)}$~如下
\begin{align}\tag{7}
\mathcal{P}_n^{(1)}=\{\{p_{n,t}\}_{n=1}^N|\{p_{n,t}\}_{n=1}^N~satisfies(3)-(6)\}
\end{align}
...
\begin{align}\tag{11}
G(P)\triangleq \sum_{n \in \mathcal{N}}\left\{f_n(p_{nt})-\omega_ng_n(\sum_{t\in \mathcal{T}}p_{nt})+\gamma\|p_n\|_1\right\}+\delta_n\|\mathbf{p}-\overline{\mathbf{p}}\|_2^2\\\nonumber
\end{align}
...
\begin{align}\tag{12}
&\min_{\{u\}}~~G(P)\\\tag{13}
&s.t.~~\sum_{n\in\Pi_n}p_{n,t}\leq c_{lt}\\\tag{14}
...
且
\begin{align*}
\mathcal{P}_t^{(2)}=\mathcal{P}_1^{(2)}\times \cdot\cdot\cdot \times \mathcal{P}_t^{(2)}\times\cdot\cdot\cdot\times \mathcal{P}_T^{(2)}\tag{16}
\end{align*}
...
现在我们可以很容易地将~$(4.1)-(4.3)$~所描述的问题写成标准的ADMM 形式，如下所示
\begin{align}\tag{18}
...
定义增广拉格朗日函数如下:
\begin{align*}
\mathcal{L}_{\rho}(P,Y,\mu)&=f(P)+g(Y)+\mu^T(P-Y)+\frac{\rho}{2}\|P-Y\|_F^2\\
&=f(P)+g(Y)+\frac{\rho}{2}\|P-Y+\frac{\mu}{\rho}\|_F^2-\frac{\rho}{2}\|\frac{\mu}{\rho}\|_F^2
\end{align*}
...
\begin{align*}\tag{23}
P^{k+1}=\mathop{argmin}\limits_{P\in\mathcal{P}^{(1)}}\{G(P)+\frac{\rho}{2}\|P-Y+\frac{\mu}{\rho}\|_F^2+\frac{\phi}{2}(P-P^k)^T(P-P^k)\}
\end{align*}
当~$n=N+1$~时
\begin{align*}\tag{24}
...
Y^{k+1}&=\mathop{argmin}\limits_{Y}~\mathcal{I}_2(Y)+\frac{\rho}{2}\|P^k-Y+\frac{\mu^k}{\rho}\|_F^2+\frac{\psi}{2}\|Y-Y^k\|_F^2\\
       &=\mathop{argmin}\limits_{Y\in\mathcal{P}^{(2)}}~\frac{\rho}{2}\|Y-(P^{k}+\frac{\mu^k}{\rho})\|_F^2+\frac{\psi}{2}\|Y-Y^k\|_F^2
\end{align*}
...
\bl{
目标函数：
$$G(\{p_{n,t}\}_{n=1,t=1}^{N,T})=\sum_{n\in \mathcal{N}}\sum_{t\in \mathcal{T}}\lambda_tp_{nt}\Delta t
+\sum_{n\in \mathcal{N}}-(\sum_{t\in \mathcal{T}}p_{n,t}-\Gamma_n)^2
+\sum_{n\in \mathcal{N}}\|\{p_{n,t}\}_{t=1}^{T}\|_0
+\sum_{t\in \mathcal{T}}(\sum_{n\in \mathcal{N}}p_{n,t}-d_t)^2$${\color{red}目标函数没有新意了}

分解成$N+1$个小问题：{\color{red}为什么这么分解}
\begin{align*}
G(\{p_{n,t}\}_{n=1,t=1}^{N,T})
&=\sum_{t\in \mathcal{T}}\lambda_tp_{1,t}\Delta t-(\sum_{t\in \mathcal{T}}p_{1,t}-\Gamma_1)^2+\|\{p_{1,t}\}_{t=1}^{T}\|_0\\
&+\sum_{t\in \mathcal{T}}\lambda_tp_{2,t}\Delta t-(\sum_{t\in \mathcal{T}}p_{2,t}-\Gamma_1)^2+\|\{p_{2,t}\}_{t=1}^{T}\|_0\\
&+\sum_{t\in \mathcal{T}}\lambda_tp_{3,t}\Delta t-(\sum_{t\in \mathcal{T}}p_{3,t}-\Gamma_1)^2+\|\{p_{3,t}\}_{t=1}^{T}\|_0\\
&+\cdots\\
&+\sum_{t\in \mathcal{T}}\lambda_tp_{N,t}\Delta t-(\sum_{t\in \mathcal{T}}p_{N,t}-\Gamma_1)^2+\|\{p_{N,t}\}_{t=1}^{T}\|_0\\
&+\sum_{t\in \mathcal{T}}(\sum_{n\in \mathcal{N}}p_{n,t}-d_t)^2
\end{align*}

引入记号：
\begin{align}
&\mathbf{p}_n:=\{p_{n,t}\}_{t=1}^T, \forall n\in\mathcal{N}; \\
&\mathbf{p}_{N+1}:=\{\sum\limits_{n=1}^N p_{n,t}\}_{t=1}^T;\\
&f_n(\mathbf{x}):=\Delta t{\lambda_t}\mathbf{x}-(\sum_{t\in \mathcal{T}}\mathbf{x}-\Gamma_1)^2+\|\mathbf{x}\|_1\\
&f_{N+1}(\mathbf{x}):=\sum_{t\in \mathcal{T}}(\mathbf{x}-d_t)^2
\end{align}

则上述问题可以等价表述为：{\color{red}约束条件没看明白}

\begin{align*}
  \textrm{minmize } f_1(\mathbf{p}_1)+\cdots+f_N(\mathbf{p}_N)+f_{N+1}(\mathbf{p}_{N+1})\\
  \textrm{subject to } \mathbf{I}\mathbf{p}_1+\cdots+\mathbf{I}\mathbf{p}_N-\mathbf{I}\mathbf{p}_{N+1}=\mathbf{0}\\
  \mathbf{p}_1\in \mathcal{X}_1,\cdots,\mathbf{p}_N\in \mathcal{X}_N,\mathbf{p}_{N+1}\in \mathcal{X}_{N+1}
\end{align*}
这里 $\mathcal{X}_1$ 表示满足了对应限制条件的空间
}
