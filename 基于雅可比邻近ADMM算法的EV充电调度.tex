...
2）设计了一种新的基于分布式\,$Jacobi-PADMM$\,的算法来解决具有时间空间耦合不等式约束的不可分优化问题,\,并从理论上证明了该算法的最优性和收敛性,\,从理论和数值实验上证明了与经典的~$ADMM$~相比收敛速度快.\,保护每位电动汽车车主的隐私.

\section{电动汽车充电模型}
引入记号:
设~$\bm{p}\in \mathbb{R}^n$,\,$\|\bm{p}\|_2=(\sum_{i=1}^np_i^2)^{1/2}$\,表示标准的欧式范数,\,$\|\bm{p}\|_0=\sharp(i|p_i\neq0)$\,表示向量\,$\bm{p}$\,中非零元的个数.\\
...
\end{align*}
在实践中,~一般采用以下保守约束,即每个~$EV n$~在有限时间范围~$T$~结束时至少达到最终SOC 值,~表示为~$SOC_n^{final}$,
\begin{align}\label{powerC}
\sum_{t=1}^Tp_{n,t}\geq \frac{C_n}{q_n^+\Delta t}(SOC_n^{final}-SOC_n^{init}).
\end{align}
为了简化表示,~定义~$\mathcal{P}_n^{(1)}$~如下
\begin{align}
\mathcal{P}_n^{(1)}=\{\{p_{n,t}\}_{n=1}^N|\{p_{n,t}\}_{n=1}^N~satisfies(\ref{SOCC})-(\ref{powerC})\}
\end{align}
...
\subsection{目标函数}
\begin{align}
G_0(\bm{p})&\triangleq\sum_{n\in \mathcal{N}}\left\{f_n(p_{nt})
...
2) 第二项$g_n(\sum\limits_{t\in \mathcal{T}}p_{nt})$:它是关于在充电期间交付的总能量的满意函数,~电动汽车用户更愿意充电时，直到他们在所需的时间内达到所需的容量~$\Gamma_n$,~然后,~我们提出以下形式表示用户的满意度:
\begin{align*}
...
\begin{align}
G(\bm{p})\triangleq \sum_{n \in \mathcal{N}}\left\{f_n(p_{nt})-\omega_ng_n(\sum_{t\in \mathcal{T}}p_{nt})+\gamma\|p_n\|_1\right\}+\delta_n\sum_{t\in \mathcal{T}}h_n(\sum_{t\in\mathcal{T}}p_{nt})\\\nonumber
\end{align}
...
值得注意的是,~在模型(11)中的第二项耦合了所有的变量~$p_{n,t},n\in \mathcal{N}$,~ 为了解耦此项,根据文献引入辅助变量
\begin{align*}
...
\begin{align}
&\min_{\{u\}}~~G(\bm{p})\label{objectf}\\
&s.t.~~\sum_{n\in\Pi_n}p_{n,t}\leq c_{lt}\label{objectfc1}\\
&~~~~~~ p_{N+1,t}=\sum\limits_{t\in \mathcal{T}}p_{nt}-\overline{\bm{p}},\,t\in \mathcal{T}\label{objectfc2}
\end{align}

为了使用\,$Jacobi-PADMM$\,,定义
\begin{align}
\mathcal{P}_t^{(2)}=\{P_t|P_t~satisfies~(\ref{objectfc1})~and~(\ref{objectfc2})\}
\end{align}
且
\begin{align}
...
现在我们可以很容易地将~$(\ref{objectf})-(\ref{objectfc2})$~所描述的问题写成标准的ADMM 形式，如下所示
\begin{align}
&\min_{\{u\}}G(P)+\mathcal{I}_1(P)+\mathcal{I}_2(Y)\\\tag{19}
&s.t~~P-Y=0.\nonumber
\end{align}
这里定义:
\begin{align*}
&f(P)\triangleq G(p)+\mathcal{I}_1(P)\\
&g(Y)\triangleq \mathcal{I}_2(Y)
\end{align*}

定义增广拉格朗日函数如下:
\begin{align}
\mathcal{L}_{\rho}(P,Y,\mu)&=f(P)+g(Y)+\Lambda^T(P-Y)+\frac{\rho}{2}\|P-Y\|_F^2\\
&=f(P)+g(Y)+\frac{\rho}{2}\|P-Y+\bm{u}\|_F^2\nonumber
\end{align}
其中~$\rho>0$~是惩罚参数,\,$\Lambda$\,称为对偶变量或者拉格朗日乘子,\,$\bm{u}$称为缩放后的对偶变量,\,i.e.$\bm{u}=(1/\rho)\Lambda$\,变量\,$p,x,u$\, 的更新

\begin{align}
&P^{k+1} \triangleq \mathop{argmin}\limits_{P}\{\mathcal{L}_{\rho}(P,Y,\bm{u}^k)+\frac{1}{2}\|P-P^k\|_{\Phi}^2\}\\
&Y^{k+1}\triangleq \mathop{argmin}\limits_{Y}\{\mathcal{L}_{\rho}(P^{k},Y,\bm{u}^k)+\frac{1}{2}\|Y-Y^k\|_\Psi^2\}\label{yupdate}\\
&\mu^{k+1}\triangleq \bm{u}^k-\zeta\rho(P^{k+1}-Y^{k+1})
\end{align}
...
\begin{align}
P^{k+1}=\mathop{argmin}\limits_{P\in\mathcal{P}^{(1)}}\{G(P)+\frac{\rho}{2}\|P-Y+\bm{u}\|_F^2+\frac{\phi}{2}(P-P^k)^T(P-P^k)\}
\end{align}
当~$n=N+1$~时
\begin{align}
...
对于每一个~n=1,...,N
\begin{align}
p_n^{k+1}=\mathop{argmin}\limits_{Y\in\mathcal{P}^{(1)}}~\{G_n(p)+\frac{\rho}{2}\|p_n-y_n+\frac{\mu_n}{\rho}\|_F^2+\frac{\psi}{2}(p_n-p_n^k)^T(p_n-p_n^k)\}
\end{align}
\section{计算Y}

本节详细介绍(\ref{yupdate})中变量\,$Y$\,的更新,\,等价于求解以下凸优化问题的解:
\begin{align}\label{subob}
Y^{k+1}&=\mathop{argmin}\limits_{Y}~\mathcal{I}_2(Y)+\frac{\rho}{2}\|P^k-Y+\bm{u}\|_F^2+\frac{\psi}{2}\|Y-Y^k\|_F^2\\
       &=\mathop{argmin}\limits_{Y\in\mathcal{P}^{(2)}}~\frac{\rho}{2}\|Y-(P^{k}+\bm{u})\|_F^2+\frac{\psi}{2}\|Y-Y^k\|_F^2\nonumber
\end{align}
%{\color{red}第一种思路}:\\
%由于(\ref{subob})中的函数是解耦的,\,因此计算其变量\,$y_i^k$\,i=1,...,N.\,的优化问题为
%\begin{align}
%\min_{y_t\in \mathcal{P}_t^{(2)}}\quad[(\rho+\psi)y_i^2-2(\rho(p_i^k+\bm{u}_i^k)+\psi y_i^k)y_i]
%s.t.
%\end{align}
...
\end{align*}
子问题(30)是一个CQP问题。
    为了简化符号,~我们引入一个~$(L\times N)$~矩阵~$A^t$~来表示时刻~$t$~的馈线用户关联矩阵及其在~$a_{l,n}^t$~处的分量
...
\begin{align*}\tag{29}
&min~~\sum_{n=1}^{N+1}y_{n,t}^2-2(x_{n,t}^k+U_{n,t}^k)y_{n,t}\\\tag{30}
&s.t.~~~~\sum_{n=1}^{N+1}a_n^ty_{n,t}\leq c^t\\\tag{31}
&~~~~~~~~\sum_{n=1}^{N+1}b_ny_{n,t}=d_t\\
\end{align*}
引入(31)的拉格朗日函数
\begin{align*}
\mathcal{L}^t(y_t;\lambda_t,\mu_t)&=\sum_{n=1}^{N+1}[y_{n,t}^2-2(x_{n,t}+U_{n,t}^k)y_{n,t}]+\lambda_t^T[a_n^ty_{n,t}-c^t/(N+1)]\\
&+\mu_t[b_ny_{n,t}-d_t/(N+1)]\\\tag{32}
...
\section{weiyifan note}
\begin{align*}\tag{20}
&P^{k+1} \triangleq \mathop{argmin}\limits_{P}\{\mathcal{L}_{\rho}(P,Y,\mu^k)+\frac{1}{2}\|P-P^k\|_{\Phi}^2\}\\\tag{21}
&Y^{k+1}\triangleq \mathop{argmin}\limits_{Y}\{\mathcal{L}_{\rho}(P^{k},Y,\mu^k)+\frac{1}{2}\|Y-Y^k\|_\Psi^2\}\\\tag{22}
...
$p_{N+1,t}:=\sum\limits_{n=1}^N p_{n,t}$

$\Gamma_n$~为电第$n$辆电动汽车的充电需求,
...
\begin{algorithm}[!h]
    ...
    \renewcommand{\algorithmicensure}{\textbf{Output:}}
    \begin{algorithmic}[1]
        \REQUIRE $\mathbf{p}_n^0, A_n(n=1,2,...,N)$,~$\lambda^0,~\varepsilon^{pri},~\rho, \gamma$ %%input
        \ENSURE $\mathbf{p}_n^k(n=1,2,...,N)$    %%output
       \FOR{k=0,1,...}
                \STATE Update~$\mathbf{p}_n$~for~$n=1,...,N$~in parallel by:
                \STATE $\mathbf{p}_n^{k+1}= \textrm{argmin }\quad f_n(\mathbf{p}_n)
                +\frac{\rho}{2}\|A_n\mathbf{p}_n+\sum_{j\neq i}A_j\mathbf{p}_j^k
                -\underbrace{c}_{c=0}
                -\frac{\lambda^k}{\rho}\|_2^2$
                \STATE \hspace{2.7cm}$+\frac{1}{2}\|\mathbf{p}_n-\mathbf{p}_n^k\|_{P_i}^2$
                \STATE Update $\lambda^{k+1}=\lambda^k-\gamma\rho(\sum_{i=1}^N A_i\mathbf{p}_n^{k+1}-c)$


                \STATE Update~$r^{k+1}=\sum_{n=1}^{N+1}\|\mathbf{p}_n^{k+1}-\mathbf{p}_n^{k}\|_F$
            \IF {$r^{k+1} \leq \varepsilon^{pri}$}
                \STATE Break.
            \ENDIF
...
\begin{algorithm}[!h]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE  使用matlab 优化工具箱，求解此带约束的二次优化函数，即可
    \end{algorithmic}
\end{algorithm}
