...
在电力系统中,~电网功率是由多个电源和负载之间的相互作用决定的.~这些电源和负载在运行过程中会受到各种因素的影响,~如天气变化、市场需求变化、设备故障等,~ 这些因素都会导致电网功率的波动.~通过对电网功率波动的统计分析,~可以计算出电网功率的方差,~方差越大说明电网功率波动越大.~如果电网功率波动过大,~就可能导致电网运行不稳定,~出现电压跌落、频率波动等问题,~甚至可能引发电网崩溃,~造成严重的电力事故.
...
优化问题重写为如下形式:
\begin{align*}
&\min_p  G(P)+\mathcal{I}_1(P)+\mathcal{I}_2(Y)\\
& s.t.\quad P-Y=0
\end{align*}
定义
\begin{align*}
\mathcal{P}_t^{(2)}=\{P_t|P_t~satisfies~(3.2)~and~(3.3)\}
\end{align*}
且
\begin{align*}
...
现在我们可以很容易地将~$(3.1)-(3.3)$~所描述的问题写成标准的ADMM形式，如下所示
\begin{align}
...
定义增广拉格朗日函数如下:
\begin{align*}
\mathcal{L}_{\rho}=G(P)+\mathcal{I}_1(P)+\mathcal{I}_2(X)+\mu^T(P-X)+\frac{\rho}{2}\|P-X\|_2^2
\end{align*}
其中~$\rho>0$~是惩罚参数,~$\mu$~称为对偶变量或者拉格朗日乘子,~变量~$p,x,u$~ 的更新
\begin{align*}
&P^{k+1} \triangleq \mathop{argmin}\limits_{Y}\mathcal{L}_{\rho}(P^k,Y,\mu^k)+\frac{1}{2}\|P-P^k\|_{\Phi}^2\\
&Y^{k+1}\triangleq \mathop{argmin}\limits_{Y}\mathcal{L}_{\rho}(P,Y^k,\mu^k)+\frac{1}{2}\|Y-Y^k\|_\Psi^2\\
&\mu^{k+1}\triangleq \mu^k-\zeta\rho(P^{k+1}-P^{k+1})
\end{align*}
...
\begin{align*}
P^{k+1}=\mathop{argmin}\limits_{P}G(P)+\frac{\rho}{2}\|P-Y+\frac{\mu}{\rho}\|_F^2+\frac{1}{2}\|P-P^k\|_{\Phi}^2
\end{align*}
当~$n=N+1$~时
\begin{align*}
p_{N+1}=\frac{\rho}{\rho-2\omega_n}(\frac{\mu_{N+1}^k}{\rho}-y^{k+1})
\end{align*}
对于每一个n
\begin{align*}
p_n=\mathop{argmin}\limits_{P}G_n(p)+\frac{\rho}{2}\|p_n-y_n+\frac{\mu_n}{\rho}\|_F^2+\frac{1}{2}(p_n-p_n^k)^T\phi(p_n-p_n^k)
\end{align*}
\section{计算Y}
重新更改
本节详细介绍()中变量~$x$~的更新,~等价于求解以下凸优化问题的解:
\begin{align*}
Y^{k+1}&=\mathop{argmin}\limits_{Y}~\mathcal{I}_2(Y)+\frac{\rho}{2}\|P-Y+\frac{\mu}{\rho}\|+\frac{1}{2}\|Y-Y^k\|_{\Psi}^2\\
       &=\mathop{argmin}\limits_{Y}~\frac{\rho}{2}\|Y-(P^{k}+\frac{\mu^k}{\rho})\|_F^2+\frac{1}{2}\|Y-Y^k\|_{\Psi}^2
\end{align*}
对于~$x-update$~的更新,~可以由以下分散方式计算变量
\begin{align*}
y_n^{k+1}=\mathop{argmin}\limits_{y}~\frac{\rho}{2}\{y_t^Ty_t-2(p_t^k+\frac{\mu_t^k}{\rho}^T)y^t\}+\frac{1}{2}(y_t-y_t^k)^T\psi(y_t-y_t^k)
\end{align*}
...
criterion~might~be~$\varepsilon^{-3}$~or~$\varepsilon^{-4}$.~A~pseudo-code~for~the~proposed Prox-JADMM~is~then~give~in~Algorithm~1.
...
%
%对于(25)中的近似准则，设PCG的第i次迭代为p(i)， Qi:= QJ(p(i);wJ)，当PCG达到s 次迭代时(理论上它应该已经找到(25)的右边的精确解)，或者当第i次迭代满足i≥1 和时，我们遵循[16]终止PCG
