...
\begin{algorithm}[!h]
    ...
    \renewcommand{\algorithmicensure}{\textbf{Output:}}
    \begin{algorithmic}[1]
        \REQUIRE $\mathbf{p}_n^0$,~$\lambda^0,~\varepsilon^{pri},~\rho, \gamma$ %%input
        \ENSURE $\mathbf{p}_n^k$    %%output
       \FOR{k=0,1,...}
                \STATE Update~$\mathbf{p}_n$~for~$n=1,...,N+1$~in parallel by:
                \STATE \qquad $\mathbf{p}_n^{k+1}= \textrm{argmin }\quad f_n(\mathbf{p}_n)
                +\frac{\rho}{2}\|A_n\mathbf{p}_n+\sum_{j\neq n}A_j\mathbf{p}_j^k
                -c
                -\frac{\lambda^k}{\rho}\|_{L^2}^2$
                \STATE \hspace{3cm}$+\frac{1}{2}\|\mathbf{p}_n-\mathbf{p}_n^k\|_{\Psi}^2$
                \STATE Update $\lambda$ by:
                \STATE \qquad $\lambda^{k+1}=\lambda^k-\gamma\rho(\sum_{i=1}^N A_i\mathbf{p}_n^{k+1}-c)$
...
\begin{align}
&\mathbf{p}_n^0 =\mathbf{0},\quad \lambda^0 =?,\quad \varepsilon^{pri}= ?,\quad \rho =?, \quad\gamma=?
\end{align}
